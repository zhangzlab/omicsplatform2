...
if(!dir.exists("step5/image")){
        dir.create("step5/image",recursive = TRUE)
}
...
ggsave("step5/image/step5_rename_umap.png",width=8,height=6)
ggsave("step5/image/step5_rename_umap.pdf",width=8,height=6)
saveRDS(data, file = "step5/step5_rename_dim.Rds")

p1 <- DimPlot(data, reduction = "umap",group.by = "group")
p2 <- DimPlot(data, reduction = "umap", group.by = "sample")
p1 + p2
ggsave("step5/image/step5_rename_umap_group_sample.pdf",width=12,height=6,dpi=600)
ggsave("step5/image/step5_rename_umap_group_sample.png",width=12,height=6,dpi=600)
...
ggsave("step5/image/step5_rename_umap_split_sample.pdf",width=numwidth,height=numheight)
ggsave("step5/image/step5_rename_umap_split_sample.png",width=numwidth,height=numheight)

class.num<-table(data$sample,data@meta.data$celltype)
write.table(class.num,"step5/step5_rename_sample_cluster_num.csv",col.names =NA,quote =FALSE,sep =",")
group.num<-table(data$group,data@meta.data$celltype)
write.table(group.num,"step5/step5_rename_group_cluster_num.csv",col.names =NA,quote =FALSE,sep =",")

data.markers <- FindAllMarkers(data, test.use = "MAST",only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.table(data.markers,"step5/step5_rename_positive_diff.csv",sep=",",row.names =FALSE, col.names =TRUE,  quote =FALSE)
...
ggsave("step5/image/step5_rename_top20_gene.pdf",width=8,height=6)
ggsave("step5/image/step5_rename_top20_gene.png",width=8,height=6)
...
ggsave("step5/image/step5_rename_group_cell_ratio.pdf",width=maxwidth,height=8)
ggsave("step5/image/step5_rename_group_cell_ratio.png",width=maxwidth,height=8)
...
p<-p+guides(fill = guide_legend(reverse = F))
p<-p+theme(legend.title = element_blank(),legend.text = element_text(size = 14), legend.key.size=unit(0.5,'cm'),legend.position = "right",legend.direction="vertical",legend.spacing.x = unit(0.5, 'cm'),legend.key.width = unit(0.5, "cm"))
ggsave("step5/image/step5_rename_sample_cell_ratio.pdf",width=maxwidth,height=8)
ggsave("step5/image/step5_rename_sample_cell_ratio.png",width=maxwidth,height=8)
...
write.table(cellsum,"step5/step5_rename_sample_celltype_number.csv",sep=",",quote=FALSE)
...
write.table(comb,"step5/step5_rename_sample_cell_ratio.csv",sep=",",quote=FALSE)
...
for( subcell in metass){
    ...
    ggsave(paste("step5/image/step5_rename_",tmp_cell,"_cell_ratio_stat.pdf",sep=""),width=5,height=5)
    ggsave(paste("step5/image/step5_rename_",tmp_cell,"_cell_ratio_stat.png",sep=""),width=5,height=5)
}
