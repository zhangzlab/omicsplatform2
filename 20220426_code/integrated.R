...
if(!dir.exists("step3/image")){
        dir.create("step3/image",recursive = TRUE)
}
...
if(method == "integrated"){
	...
	ggsave("step3/image/step3_combined_JackStrawPlot.pdf",width=8,height=6,dpi=600)
	ggsave("step3/image/step3_combined_JackStrawPlot.png",width=8,height=6,dpi=600)
	ElbowPlot(data.combined,ndims = 30)
	ggsave("step3/image/step3_combined_Elbowplot.pdf",width=8,height=6,dpi=600)
	ggsave("step3/image/step3_combined_Elbowplot.png",width=8,height=6,dpi=600)
	...
	saveRDS(data.combined, file = "step3/step3_comb_dimreduce.Rds")
...
ggsave("step3/image/step3_raw_umap.pdf",width=8,height=6,dpi=600)
ggsave("step3/image/step3_raw_umap.png",width=8,height=6,dpi=600)

p1 <- DimPlot(data.combined, reduction = "umap", group.by = "group")
p2 <- DimPlot(data.combined, reduction = "umap", group.by = "sample")
p1 + p2
ggsave("step3/image/step3_umap_group_sample.pdf",width=12,height=6,dpi=600)
ggsave("step3/image/step3_umap_group_sample.png",width=12,height=6,dpi=600)
...
ggsave("step3/image/step3_umap_split_sample.pdf",width=numwidth,height=numheight)
ggsave("step3/image/step3_umap_split_sample.png",width=numwidth,height=numheight)

class.num<-table(data.combined$sample,data.combined@meta.data$seurat_clusters)
write.table(class.num,"step3/step3_sample_cluster_num.csv",col.names =NA,quote =FALSE,sep =",")

DefaultAssay(data.combined) <- "RNA"

data.combined.markers <- FindAllMarkers(data.combined, test.use = "MAST",only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write.table(data.combined.markers,"step3/step3_exp_gene_diff_pos.csv",sep=",",row.names =FALSE,col.names =TRUE, quote =FALSE)
...
if(method == "integrated"){
	...
	ggsave("step3/image/step3_top20_gene_Heatmap.pdf",width=9,height=12,dpi=600)
	ggsave("step3/image/step3_top20_gene_Heatmap.png",width=9,height=12,dpi=600)
