...
if(!dir.exists("step7/image")){
        dir.create("step7/image",recursive = TRUE)
}

inputFile = opt$diff
print(inputFile)
fileType = unlist(strsplit(inputFile,split="\\."))[-1]
if (fileType %in% c("txt","tsv","xls") ) {
  sep="\t"
}else{
...
data<-read.table(file_path_as_absolute(inputFile),sep=sep,header=T)
...
write.table(as.data.frame(ALL@result), file="step7/all_GO.csv",quote=FALSE,sep=",",col.names=TRUE,row.names=FALSE)
...
ggsave("step7/image/CC_function.pdf",width=15,height=height_cc,useDingbats=FALSE)
ggsave("step7/image/CC_function.png",width=15,height=height_cc)
...
p <- p + scale_y_continuous(limits =c(0,maxvalue+1),breaks=seq(0,maxvalue+1,2),labels=seq(0,maxvalue+1,2))
p <- p + guides(fill=FALSE)
ggsave("step7/image/BP_function.pdf",width=15,height=height_bp,useDingbats=FALSE)
ggsave("step7/image/BP_function.png",width=15,height=height_bp)
...
p <- p + scale_y_continuous(limits =c(0,maxvalue+1),breaks=seq(0,maxvalue+1,2),labels=seq(0,maxvalue+1,2))
p <- p + guides(fill=FALSE)
ggsave("step7/image/MF_function.pdf",width=15,height=height_mf,useDingbats=FALSE)
ggsave("step7/image/MF_function.png",width=15,height=height_mf)

write.table(as.data.frame(CC_simp@result),file="step7/CC_GO.csv",quote=FALSE,sep=",",col.names=TRUE,row.names=FALSE)
write.table(as.data.frame(BP_simp@result),file="step7/BP_GO.csv",quote=FALSE,sep=",",col.names=TRUE,row.names=FALSE)
write.table(as.data.frame(MF_simp@result),file="step7/MF_GO.csv",quote=FALSE,sep=",",col.names=TRUE,row.names=FALSE)
...
p <- p + scale_y_continuous(limits =c(0,maxvalue+1),breaks=seq(0,maxvalue+1,2),labels=seq(0,maxvalue+1,2))
p <- p + guides(fill=FALSE)
ggsave("step7/image/kegg_function.pdf",width=15,height=height_kegg,useDingbats=FALSE)
ggsave("step7/image/kegg_function.png",width=15,height=height_kegg)

write.table(as.data.frame(keggs@result), file="step7/KEGG.csv",quote=FALSE,sep=",",col.names=TRUE,row.names=FALSE)
